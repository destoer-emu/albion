...
void Display::render_palette(u32 *palette, size_t size)
{
    for(size_t i = 0; i < size; i++)
    {
        palette[i] = convert_color(handle_read<u16>(mem.pal_ram,i*2));
    }   
}


void Display::render_map(int id, std::vector<u32> &map)
{
    ...
    const u32 y_size_table[] = {256,256,512,512};
    const u32 x_size_table[] = {256,512,256,512};
    ...
    for(u32 y = 0; y < height; y++)
    {
        ...
        u32 bg_map_base = (cnt.screen_base_block * 0x800) + (map_y % 0x20) * 64; // (2 * 32);
        ...
        for(u32 x = 0; x < width; x += 8)
        {
            ...
            // at that point
            u32 bg_map_offset = (map_x++ % 0x20) * 2; 
